...
class YRenderer 
{
	...
	public :
		...
		/**
		Si on passe forThisShaderOnly, alors on checke si c'est ce shader a qui on a envoyé les matrices a la boucle d'avant.
		Si c'est le cas on connait les matrices bindées et donc on peut limiter les calculs
		*/
		void updateMatricesFromOgl(YCamera * otherCam = NULL, GLuint forThisShaderOnly = 0,  bool updateProj = true, bool updateView = true) {
			float matMvTab[16];
			...
			if (updateView) {
				...
				if (otherCam)
					cam = otherCam;
				MatV.createViewMatrix(cam->Position, cam->LookAt, cam->UpVec);
			...
			if (forThisShaderOnly == 0 || forThisShaderOnly != LastProg || AdrMatIM >= 0) {
				MatIM = MatM;
				MatIM.invert();
			}
			
			if (forThisShaderOnly == 0 || forThisShaderOnly != LastProg || AdrMatNorm >= 0) {
				MatNorm = MatM;
				MatNorm.invert();
				MatNorm.transpose();
			}
		...
		void sendNearFarToShader(int prog, YCamera * cam = NULL) {
			if (prog == CURRENT_SHADER)
				glGetIntegerv(GL_CURRENT_PROGRAM, &prog);
			if (prog == 0)
				return;
			...
			if(!cam)
				...
				glUniform2f(var, (float)(cam->Near), (float)(cam->Far));
		}

		void sendMatricesToShader(int prog)
		{
			
			checkGlError("start sendMatricesToShader");
			...
			if (LastProg != prog) {
				LastProg = prog;
				AdrMatMVP = glGetUniformLocation(prog, "mvp");
				AdrMatMV = glGetUniformLocation(prog, "mv");
				AdrMatM = glGetUniformLocation(prog, "m");
				AdrMatV = glGetUniformLocation(prog, "v");
				AdrMatP = glGetUniformLocation(prog, "p");
				AdrMatIM = glGetUniformLocation(prog, "im");
				AdrMatIV = glGetUniformLocation(prog, "iv");
				AdrMatIP = glGetUniformLocation(prog, "ip");
				AdrMatNorm = glGetUniformLocation(prog, "nmat");
			}
						
			if (AdrMatMVP >= 0) glUniformMatrix4fv(AdrMatMVP, 1, true, MatMVP.Mat.t);
			if (AdrMatMV >= 0) glUniformMatrix4fv(AdrMatMV, 1, true, MatMV.Mat.t);
			if (AdrMatM >= 0) glUniformMatrix4fv(AdrMatM, 1, true, MatM.Mat.t);
			if (AdrMatV >= 0) glUniformMatrix4fv(AdrMatV, 1, true, MatV.Mat.t);
			if (AdrMatP >= 0) glUniformMatrix4fv(AdrMatP, 1, true, MatP.Mat.t);
			if (AdrMatIM >= 0) glUniformMatrix4fv(AdrMatIM, 1, true, MatIM.Mat.t);
			if (AdrMatIV >= 0) glUniformMatrix4fv(AdrMatIV, 1, true, MatIV.Mat.t);
			if (AdrMatIP >= 0) glUniformMatrix4fv(AdrMatIP, 1, true, MatIP.Mat.t);
			if (AdrMatNorm >= 0) glUniformMatrix4fv(AdrMatNorm, 1, true, MatNorm.Mat.t);

			checkGlError("Fin sendMatricesToShader");
		}
