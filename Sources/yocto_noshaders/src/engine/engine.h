...
public:
	...
	//Textures
	std::vector<YTexFile*> Textures;
	string RepTex = "textures/";
	string NameTexBase = "TexCustom_";
...
public :
	...
	static void updateBase(void) {
		...
		{
			Instance->GPUPartTime = Instance->TimerGPURender.getAccumTimeSec() / 1.0;
			float GPUPcent = floor(Instance->GPUPartTime * 100);
			Instance->LblFps->Text = std::string("FPS : ") +
				toString(Instance->FpsNbFrames) + 
				string(" (") + toString(floor(1000.0/Instance->FpsNbFrames)) + string("ms)") +
	...
	void loadAllTextures() {
		for (int i = 0; i < 5; i++) {
			string file = RepTex + NameTexBase + toString(i) + TexExtension;
			FILE * fe = NULL;
			fopen_s(&fe,file.c_str(), "r");
			if (fe) {
				fclose(fe);
				YTexFile * tex = YTexManager::getInstance()->loadTexture(file);
				Textures.push_back(tex);
				YLog::log(YLog::ENGINE_INFO, (file + " loaded !").c_str());
			}
		}
	}

	void sendAllTexToShader(GLint shader = 0)
	{
		int iTexInShader = 0;
		for (int i = 0; i < Textures.size(); i++)
		{
			iTexInShader += Textures[i]->setAsShaderInput(shader,
				GL_TEXTURE4 + iTexInShader,
				(NameTexBase + toString(i)).c_str());
		}
	}

	void sendTimeToShader(int prog) {
		if (prog == YRenderer::CURRENT_SHADER)
			glGetIntegerv(GL_CURRENT_PROGRAM, &prog);
		if (prog == 0)
			return;

		GLuint var = glGetUniformLocation(prog, "elapsed");
		glUniform1f(var, (float)(DeltaTimeCumul));
	}
