...
public :		
	...
	void init() 
	{
		...
		for (int i = 0; i < VboRand->getNbVertices(); i++) {
			VboRand->setElementValue(0, i, randf(), randf(), randf());
			VboRand->setElementValue(1, i, randf(), randf(), randf(), 1);
			VboRand->setElementValue(2, i, randf(), randf(), randf());
		}
			...
			else
			{
				if (MouseBtnState & GUI_MRBUTTON)
				{
					...
					if (MouseBtnState & GUI_MMBUTTON)
					{
						...
						strafe.Z = 0;
						strafe.normalize();
						strafe *= (float)dx / 300.0f;

						Renderer->Camera->move(Renderer->Camera->UpRef * (float)-dy / 300.0f);
